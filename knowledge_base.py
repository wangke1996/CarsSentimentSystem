...
class KnowledgeBase:
    ...
    def load_knowledge_base(self, entity_word_dir, attribute_word_dir, description_word_dir, whole_part_dir,
        ...
        # self.write_knowledge_base(entity_word_dir, attribute_word_dir, description_word_dir, whole_part_dir,
        #                           entity_attribute_dir, target_description_sentiment_dir, entity_synonym_dir,
        #                           attribute_synonym_dir)
    ...
    def write_knowledge_base(self, entity_word_dir, attribute_word_dir, description_word_dir, whole_part_dir,
                             entity_attribute_dir, target_description_sentiment_dir, entity_synonym_dir,
                             attribute_synonym_dir):
        def write_word(file_path, word_set):
            with open(file_path, 'w', encoding='utf8') as f:
                for word in word_set:
                    f.write(word + '\n')

        write_word(entity_word_dir, self.entitySet)
        write_word(attribute_word_dir, self.attributeSet)
        write_word(description_word_dir, self.descriptionSet)

        def write_pair(file_path, pair_dict):
            with open(file_path, 'w', encoding='utf8') as f:
                for key, values in pair_dict.items():
                    for value in values:
                        f.write(key + '\t' + value + '\n')

        write_pair(whole_part_dir, self.pairWP)
        write_pair(entity_synonym_dir, self.pairES)
        write_pair(attribute_synonym_dir, self.pairAS)
        write_pair(entity_attribute_dir, self.pairEA)

        def write_sentiment_pair():
            with open(target_description_sentiment_dir, 'w', encoding='utf8') as f:
                for target, descriptions in self.pairTD.items():
                    for description in descriptions:
                        f.write(target + '\t' + description + '\t' + self.pairSentiment[(target, description)] + '\n')

        write_sentiment_pair()
    ...
    def write_js_variables(self, over_write=False):
        if over_write or not os.path.exists(os.path.join(self.js_cache_path, 'allWords.js')):
            self.write_js_file([list(self.entitySet), list(self.attributeSet), list(self.descriptionSet),
                                # list(itertools.chain.from_iterable(self.pairES.values())),
                                self.pairSE, self.pairSA],
                               ['allEntity', 'allAttribute', 'allDescription', 'allEntitySynonym',
                                'allAttributeSynonym'], 'allWords', over_write)
        ...
        # ---- Description information (Targets and Sentiment)) ---- #
    ...
    def write_description_info(self, description, over_write=True):
        file_path = os.path.join(self.js_cache_path, description + '.js')
        if over_write is False and os.path.exists(file_path) is True:
            return
...
class KnowledgeDataBase:
    def __init__(self, config_path, js_cache_path, product_name="汽车", db_type='local'):
        if db_type == 'local':
            config_file = os.path.join(config_path, product_name + '_local')
        else:
            config_file = os.path.join(config_path, product_name)
        with open(config_file, 'rb') as f:
            ...
            usr = pickle.load(f)
            psw = pickle.load(f)
            session_id = self.client.connect(usr, psw)
            db_name = pickle.load(f)
            usr = pickle.load(f)
            psw = pickle.load(f)
            db = self.client.db_open(db_name, usr, psw)
        self.productName = product_name
        self.js_cache_path = os.path.join(js_cache_path, product_name)
    ...
    def write_js_variables(self, over_write=False):
        # ---- Entity information (Attributes, Synonyms, and (Description, Sentiment) pairs) ---- #
        entities = [x.name for x in self.client.query("select name from Entity", -1)]
        for _entity in entities:
            self.write_entity_info(_entity, over_write)
        # ---- Attribute information (Synonyms and (Description, Sentiment) pairs) ---- #
        attributes = [x.name for x in self.client.query("select name from Attribute", -1)]
        for _attribute in attributes:
            self.write_attribute_info(_attribute, over_write)
        # ---- Description information ((Targets, Sentiment) pairs) ---- #
        descriptions = [x.name for x in self.client.query("select name from Description", -1)]
    ...
    def build_target_sentiment_tree(self, target, target_type):
        target_node = {'name': target, 'child': [], 'id': 1, 'type': target_type}
        pos_node = {'name': '正向描述', 'child': [], 'id': 2, 'type': 'describe'}
        neu_node = {'name': '中性描述', 'child': [], 'id': 3, 'type': 'describe'}
        neg_node = {'name': '负向描述', 'child': [], 'id': 4, 'type': 'describe'}
        ...
        if target_type == "entity":
            ...
            edge = 'ATTDES'
        sentiment_results = self.client.query("select name,in.name from %s where out.name='%s'" % (edge, target), -1)
    ...
    def write_entity_info(self, entity, over_write=True):
        ...
        synonyms = self.synonyms_of_entity(entity)
        fathers = self.father_of_entity(entity)
        children = self.children_of_entity(entity)
        attributes = self.attributes_of_entity(entity, False)
        descriptions = self.descriptions_of_target(entity, with_sentiment_flag=True)
        print('write entity info: %s, fathers: %d, children: %d, attributes: %d, descriptions: %d' % (
            entity, len(fathers), len(children), len(attributes), len(descriptions)))
        synonym_group = {'name': "synonyms",
                         'children': [{'name': x, 'type': "entity_synonym"} for x in synonyms],
                         'type': "group"}
        father_group = {'name': "fathers", 'children': [{'name': x, 'type': "entity"} for x in fathers],
                        'type': "group"}
        children_group = {'name': "children", 'children': [{'name': x, 'type': "entity"} for x in children],
                          'type': "group"}
        attribute_group = {'name': "attributes",
                           'children': [{'name': x, 'type': "attribute"} for x in attributes],
                           'type': "group"}
        pos_description_group = {'name': "positive",
                                 'children': [{'name': x[0], 'type': "positive_description", 'sentiment': "POS"}
                                              for x in descriptions
                                              if x[1] == "POS"],
                                 'type': "group"}
        neu_description_group = {'name': "neutral",
                                 'children': [{'name': x[0], 'type': "neutral_description", 'sentiment': "NEU"}
                                              for x in descriptions
                                              if x[1] == "NEU"],
                                 'type': "group"}
        neg_description_group = {'name': "negative",
                                 'children': [{'name': x[0], 'type': "negative_description", 'sentiment': "NEG"}
                                              for x in descriptions
                                              if x[1] == "NEG"],
                                 'type': "group"}
        description_group = {'name': "descriptions",
        ...
        self.write_js_file([entity_graph],
                           ['partial_graph'], entity,
                           over_write)
    ...
    def write_attribute_info(self, attribute, over_write=True):
        ...
        synonyms = self.synonyms_of_attribute(attribute)
        entities = self.entities_of_attribute(attribute)
        descriptions = self.descriptions_of_target(attribute, with_sentiment_flag=True)
        print(
            'write attribute info: %s, entities: %d, descriptions: %d' % (attribute, len(entities), len(descriptions)))
        synonym_group = {'name': "synonyms",
                         'children': [{'name': x, 'type': "attribute_synonym"} for x in synonyms],
                         'type': "group"}
        entity_group = {'name': "entities", 'children': [{'name': x, 'type': "entity"} for x in entities],
                        'type': "group"}
        pos_description_group = {'name': "positive",
                                 'children': [{'name': x[0], 'type': "positive_description", 'sentiment': "POS"}
                                              for x in descriptions
        ...
        self.write_js_file([attribute_graph], ['partial_graph'], attribute, over_write)
    ...
    def write_description_info(self, description, over_write=True):
        ...
        neu_target_group = {'name': "neutral", 'children': [], 'type': "group"}
        neg_target_group = {'name': "negative", 'children': [], 'type': "group"}
        targets = self.targets_of_description(description, with_sentiment_flag=True)
        print('write description info: %s, targets: %d' % (description, len(targets)))
        for target in targets:
            target_type = target[2]
            sentiment = target[1]
            target_name = target[0]
            if sentiment == "POS":
                pos_target_group['children'].append({'name': target_name, 'type': target_type, 'sentiment': "POS"})
            if sentiment == "NEU":
                neu_target_group['children'].append({'name': target_name, 'type': target_type, 'sentiment': "NEU"})
            if sentiment == "NEG":
                neg_target_group['children'].append({'name': target_name, 'type': target_type, 'sentiment': "NEG"})
        description_graph = [pos_target_group, neu_target_group, neg_target_group]
        self.write_js_file([description_graph], ['partial_graph'], description, over_write)
    ...
    def synonyms_of_entity(self, entity, check_hint=True):
        ...
        """
        if self.have_entity(entity, False):
            return [x._in for x in self.client.query("select in.name from ESYN where out.name='%s'" % entity, -1)]
        else:
            # check failure
            if check_hint:
                print("query synonym failed! %s is not an entity" % entity)
    ...
    def synonyms_of_attribute(self, attribute, check_hint=True):
        ...
        """
        if self.have_attribute(attribute, False):
            return [x._in for x in self.client.query("select in.name from ASYN where out.name='%s'" % attribute, -1)]
        else:
            # check failure
            if check_hint:
                print("query synonym failed! %s is not an attribute" % attribute)
    ...
    def entity_of_synonym(self, synonym, check_hint=True):
        ...
        if len(self.client.query("select from Entity where name='%s'" % synonym)) > 0:
            return synonym
        else:
            origin_entity = self.client.query("select out.name from ESyn where in.name='%s'" % synonym)
            if len(origin_entity) > 0:
                return origin_entity[0]._out
            else:
                # check failure
                if check_hint is True:
                    print("query entity failed! %s is neither an entity nor an entity synonym" % synonym)
    ...
    def attribute_of_synonym(self, synonym, check_hint=True):
        ...
        :return: (string) the attribute of the input synonym word
        """
        if len(self.client.query("select from Attribute where name='%s'" % synonym)) > 0:
            return synonym
        else:
            origin_attribute = self.client.query("select out.name from ASYN where in.name='%s'" % synonym)
            if len(origin_attribute) > 0:
                return origin_attribute[0]._out
            else:
                # check failure
                if check_hint is True:
                    print("query entity failed! %s is neither an entity nor an entity synonym" % synonym)
    ...
    def children_of_entity(self, entity, include_synonym_flag=False, check_hint=True):
        ...
        if origin_entity is None:
            ...
            if check_hint:
                print("query child-entity failed! %s is not an entity" % entity)
            return None
        child_entities = [x._in for x in
                          self.client.query("select in.name from ENTENT where out.name='%s'" % origin_entity, -1)]
        if include_synonym_flag is True:
            synonyms = []
            for child in child_entities:
                synonyms.extend(self.synonyms_of_entity(child))
    ...
    def father_of_entity(self, entity, include_synonym_flag=False, check_hint=True):
        ...
        if origin_entity is None:
            if check_hint:
                print("query father-entity failed! %s is not an entity" % entity)
            return None
        father_entities = [x._out for x in
                           self.client.query("select out.name from ENTENT where in.name='%s'" % origin_entity, -1)]
        if include_synonym_flag is True:
            synonyms = []
            for father in father_entities:
                synonyms.extend(self.synonyms_of_entity(father))
    ...
    def attributes_of_entity(self, entity, include_synonym_flag=False, check_hint=True):
        ...
        if origin_entity is None:
            ...
            if check_hint:
                print("query attributes failed! %s is not an entity" % entity)
            return None
        attributes = [x._in for x in
                      self.client.query("select in.name from ENTATT where out.name='%s'" % origin_entity, -1)]
        if include_synonym_flag is True:
            synonyms = []
            for attribute in attributes:
                synonyms.extend(self.synonyms_of_attribute(attribute))
    ...
    def entities_of_attribute(self, attribute, include_synonym_flag=False, check_hint=True):
        ...
        if origin_attribute is None:
            ...
            if check_hint:
                print("query entities failed! %s is not an attribute" % attribute)
            return None
        entities = [x._out for x in
                    self.client.query("select out.name from ENTATT where in.name='%s'" % origin_attribute, -1)]
        if include_synonym_flag is True:
            synonyms = []
            for entity in entities:
                synonyms.extend(self.synonyms_of_entity(entity))
    ...
    def sentiment_of_target_description_pair(self, target, description, check_hint=True):
        ...
        if origin_target is None:
            ...
            if check_hint is True:
                print("query sentiment for pair(%s,%s) failed! %s is not a target" % (target, description, target))
            return None
        target_type = self.target_type(origin_target)
        if target_type == 'entity':
            edge = 'ENTDES'
        else:
            edge = 'ATTDES'
        seniment = self.client.query(
            "select name from %s where in.name='%s' and out.name='%s'" % (edge, description, origin_target))
        if len(seniment) > 0:
            return seniment[0].name
        # check failure
        if check_hint is True:
            print("query sentiment for pair(%s,%s) failed! they are not a pair" % (target, description))
        return None

    def descriptions_of_target(self, target, with_sentiment_flag=False, check_hint=True):
        """
        return descriptions of the input target (synonym) word, if target is not in the knowledge base, return None
        :param target: (string) input target
        :param check_hint: (bool) whether to print check info
        ...
        if target_type == 'entity':
            ...
            edge = 'ATTDES'
        if not with_sentiment_flag:
            return [x._in for x in
                    self.client.query("select in.name from %s where out.name='%s'" % (edge, origin_target), -1)]
        else:
            return [(x._in, x.name) for x in
                    self.client.query("select name,in.name from %s where out.name='%s'" % (edge, origin_target), -1)]

    def targets_of_description(self, description, include_synonym_flag=False, with_sentiment_flag=False,
                               check_hint=True):
        """
        return set of (target, sentiment) of the input description word, if description is not in the knowledge base, return None
        :param description: (string) input description word
        ...
        if not with_sentiment_flag:
            ...
            targets = [x._out for x in
                       self.client.query("select out.name from ATTDES where in.name='%s'" % description, -1)] \
                      + [x._out for x in
                         self.client.query("select out.name from ENTDES where in.name='%s'" % description, -1)]
            if include_synonym_flag is True:
                synonyms = []
                for target in targets:
                    synonyms.extend(self.synonyms_of_target(target))
            ...
            # targets=[(target1,sentiment1,target_type1), (target2,sentiment2,target_type2), ...]
            targets = [(x.out, x.name, x.out2) for x in
                       self.client.query("select name,out.name,out.@class from ATTDES where in.name='%s'" % description,
                                         -1)] \
                      + [(x.out, x.name, x.out2) for x in self.client.query(
                "select name,out.name,out.@class from ENTDES where in.name='%s'" % description, -1)]
            if include_synonym_flag is True:
                synonyms = []
                for target in targets:
                    synonyms.extend([(x, target[1], target[2]) for x in self.synonyms_of_target(target[0])])
                targets.extend(synonyms)
        return targets
...
def knowledge_data_base_init(product='汽车', db_type='local'):
