...
class SentimentAnalysis:
    def __init__(self):
        # load data
        print('Loading data...')
        with open(os.path.join(PRE_PATH, 'data', 'word_set.pkl'), 'rb') as f:
            self.word_set = pickle.load(f)
        with open(os.path.join(PRE_PATH, 'data', 'word_embedding.pkl'), 'rb') as f:
            self.word_embedding = pickle.load(f)
        with open(os.path.join(PRE_PATH, 'data', 'knowledge_base.pkl'), 'rb') as f:
        ...
        # load model
        print('Loading model...')
        self.model_PAIR = torch.load(os.path.join(PRE_PATH, 'model', 'model_CNN_PAIR_18'))
        self.model_SENT = torch.load(os.path.join(PRE_PATH, 'model', 'model_MNKG_SENT_19'))
